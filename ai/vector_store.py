...
class VectorStore:
    ...
    def _conn(self):
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("CREATE TABLE IF NOT EXISTS vectorstore_edges (source_id TEXT, target_id TEXT)")
            conn.commit()
        return conn
    ...
    def add_edge(self, source_id, target_id):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("INSERT INTO vectorstore_edges (source_id, target_id) VALUES (%s, %s)",
                            (source_id, target_id))
            conn.commit()
        finally:
            conn.close()

    def neighbors(self, ids, limit=20):
        ids = list(ids)
        if not ids:
            return []
        conn = self._conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT c.id, c.metadata, 0.0 FROM vectorstore_edges e "
                    "JOIN vectorstore_chunks c ON c.id = e.target_id "
                    "WHERE e.source_id = ANY(%s) LIMIT %s", (ids, limit))
                rows = cur.fetchall()
            return [{"id": r[0], "metadata": r[1], "distance": 0.0} for r in rows]
        finally:
            conn.close()
